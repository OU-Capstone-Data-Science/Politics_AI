...
def scrape_tweets():
    ...
    while True:
        # the database will be deleted periodically to minimize computation time and disk storage
        database_creation_time_hours = datetime.datetime.now().hour
        database_time_alive_hours = datetime.datetime.now().hour
        ...
        def worker():
            while True:
                try:
                    ...
                    worker_connection = sqlite3.connect(database_path, check_same_thread=False)
        ...
        print("\n*****  Recreating database  *****\n")
        os.remove(os.path.relpath(database_path))
